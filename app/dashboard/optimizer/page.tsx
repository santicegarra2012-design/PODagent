  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8 pb-20">
      ...
      <div className="space-y-2">
        ...
        <h1 className="text-3xl font-bold text-slate-900">Listing Optimizer</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_400px] gap-8 items-start">
        ...
        <div className="space-y-6">
          <div className="glass border-slate-200 rounded-3xl p-8 space-y-6">
            {/* Platform Selector */}
                  ...
                  <button
                    ...
                    className={cn(
                      ...
                      listing.platform === p
                        ...
                        : "text-slate-500 hover:text-slate-700 border-slate-100 bg-slate-50"
                    )}
                  >
                    {p}
                  </button>
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                ...
                <span className={cn(
                  ...
                  listing.title.length > 140 ? "text-red-400" : "text-slate-500"
                )}>
              ...
              <input
                ...
                className="w-full bg-slate-50 border border-slate-200 rounded-2xl p-4 text-slate-900 placeholder-slate-400 focus:ring-1 focus:ring-primary/50 transition-all"
              />
            </div>

            {/* Tags Input */}
            <div className="space-y-3">
              <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Tags / Keywords</label>
              <div className="flex flex-wrap gap-2 mb-3">
                {listing.tags.map((tag) => (
                  <span key={tag} className="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-primary/10 border border-primary/20 text-xs text-primary font-medium">
                    {tag}
                    <button onClick={() => removeTag(tag)} className="hover:text-slate-700 transition-colors">
              ...
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 bg-slate-50 border border-slate-200 rounded-2xl p-4 text-slate-900 placeholder-slate-400 focus:ring-1 focus:ring-primary/50 transition-all"
                />
                <button 
                  onClick={(e) => { e.preventDefault(); addTag(); }}
                  className="px-6 rounded-2xl bg-slate-50 border border-slate-200 text-slate-700 hover:bg-slate-100 transition-all"
                >
                  Add
                </button>
              </div>
            ...
            {/* Description Input */}
            <div className="space-y-3">
              <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Listing Description</label>
              <textarea
                value={listing.description}
                onChange={(e) => setListing({ ...listing, description: e.target.value })}
                placeholder="Paste your listing description here..."
                rows={8}
                className="w-full bg-slate-50 border border-slate-200 rounded-2xl p-4 text-slate-900 placeholder-slate-400 focus:ring-1 focus:ring-primary/50 transition-all resize-none"
              />
            </div>
          ...
          <AnimatePresence mode="wait">
            {isLoading ? (
              <motion.div
                ...
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
                className="glass border-slate-200 rounded-3xl p-12 flex flex-col items-center justify-center text-center space-y-4"
              >
                <div className="w-16 h-16 rounded-full border-4 border-primary/20 border-t-primary animate-spin" />
                <div>
                  <h3 className="text-slate-900 font-bold">Analyzing Competition</h3>
                  <p className="text-slate-500 text-xs">Finding high-conversion keywords...</p>
                </div>
              </motion.div>
            ) : result ? (
              <motion.div
              ...
              >
                {/* Scores */}
                <div className="glass border-slate-200 rounded-3xl p-8 grid grid-cols-2 gap-4">
                ...
                {/* Suggestions Card */}
                <div className="glass border-slate-200 rounded-3xl p-6 space-y-4">
                  ...
                  <div className="space-y-3">
                    <p className="text-[11px] font-bold text-slate-500 uppercase tracking-wider">Opportunity Keywords</p>
                ...
                <div className="grid grid-cols-1 gap-2">
                  <button className="flex items-center justify-between p-4 rounded-2xl bg-slate-50 border border-slate-200 text-slate-700 hover:bg-slate-100 transition-all text-xs font-bold group">
                    <span className="flex items-center gap-3">
                      <Copy className="w-4 h-4 text-slate-400 group-hover:text-primary transition-colors" />
                      Apply Optimizations
                    </span>
                    <ArrowRight className="w-3.5 h-3.5 text-slate-300 group-hover:text-slate-900 transition-all" />
                  </button>
                  <button className="flex items-center justify-between p-4 rounded-2xl bg-slate-50 border border-slate-200 text-slate-700 hover:bg-slate-100 transition-all text-xs font-bold group">
                    <span className="flex items-center gap-3">
              ...
              <motion.div
                ...
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                className="glass border-slate-200 rounded-3xl p-12 text-center flex flex-col items-center justify-center space-y-5 border-dashed"
              >
                <div className="w-20 h-20 rounded-full bg-slate-50 flex items-center justify-center">
                  <BarChart3 className="w-8 h-8 text-slate-400" />
                </div>
                <div className="space-y-1">
                  <h3 className="text-slate-900 font-bold">Analysis Ready</h3>
                  <p className="text-slate-500 text-xs">Fill in the listing details and run the optimizer to see SEO scores.</p>
                </div>
              </motion.div>
            )}
          </AnimatePresence>
        ...
        {result && (
          ...
          >
            ...
            <div className="space-y-4">
              <h3 className="text-xs font-bold text-slate-500 uppercase tracking-widest ml-1">Current Listing</h3>
              <div className="glass border-slate-100 rounded-3xl p-8 opacity-60 grayscale-[0.5]">
                <h4 className="text-lg font-bold text-slate-900 mb-4 line-through decoration-red-500/50">{listing.title}</h4>
                <div className="flex flex-wrap gap-2 mb-6">
                  {listing.tags.map(t => <span key={t} className="px-2 py-1 rounded bg-slate-50 border border-slate-200 text-[10px] text-slate-500">{t}</span>)}
                </div>
                <p className="text-sm text-slate-500 whitespace-pre-wrap">{listing.description}</p>
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="glass border-primary/20 rounded-3xl p-8 relative overflow-hidden">
                ...
                <h4 className="text-lg font-bold text-slate-900 mb-4 pr-10">{result.improvedTitle}</h4>
                <div className="flex flex-wrap gap-2 mb-6">
                ...
                <p className="text-sm text-slate-600 whitespace-pre-wrap leading-relaxed">{result.improvedDescription}</p>
              </div>
            </div>
          </motion.div>
        )}
