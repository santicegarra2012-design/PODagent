...
function Toggle({
  ...
  return (
    <button
      ...
      onClick={() => setOn((v) => !v)}
      className={cn(
        "relative w-10 h-5 rounded-full transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-primary/50",          on ? "bg-primary" : "bg-slate-200"
      )}
    >
      <span
        className={cn(
...
function SettingsSection({
  ...
  return (
    <motion.div
      ...
      className="glass border-slate-200 rounded-2xl overflow-hidden"
    >
    ...
    <div className="max-w-2xl mx-auto space-y-4">
      ...
      >
        <div className="flex items-center justify-between py-2">
          <div>
            <p className="text-sm text-slate-800 font-medium">Current Plan</p>
            <div className="flex items-center gap-2 mt-1">
              <span className={cn(
                "text-[10px] px-2 py-0.5 rounded-full font-bold uppercase tracking-wider border",
                isPro ? "bg-primary/10 border-primary/20 text-primary" : "bg-slate-50 border-slate-200 text-slate-500"
              )}>
                {planName}
              </span>
          ...
          <button 
            ...
            className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-slate-50 border border-slate-200 text-xs text-slate-600 hover:bg-slate-100 transition-colors disabled:opacity-50"
          >
            {loadingPortal ? (
